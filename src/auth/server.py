...
def login():
    ...
    res = curr.execute("SELECT email,password FROM user WHERE email=%s", (auth.username,))
...
def createJWT(username, secret, authz):
    return jwt.encode(
        {
            "username": username,
            "exp": datetime.datetime.now(tz=datetime.timedzone.utc)+ datetime.timedelta(days=1),
